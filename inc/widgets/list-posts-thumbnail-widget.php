<?php
if ( ! class_exists( 'Infinite_News_List_Posts_Thumbnail_Widget' ) ) {
	/**
	 * Adds Infinite_News_List_Posts_Thumbnail_Widget Widget.
	 */
	class Infinite_News_List_Posts_Thumbnail_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$infinite_news_list_posts_thumbnail_widget_ops = array(
				'classname'   => 'thumbnail-list-post ascendoor-widget style-1',
				'description' => __( 'Retrive List Posts Thumbnail Widgets', 'infinite-news' ),
			);
			parent::__construct(
				'infinite_news_list_posts_thumbnail_widget',
				__( 'Ascendoor List Posts Thumbnail Widget', 'infinite-news' ),
				$infinite_news_list_posts_thumbnail_widget_ops
			);
		}
				...
				<div class="thumbnail-list-post-wrapper">
					...
					if ( $query->have_posts() ) {
						while ( $query->have_posts() ) :
							$query->the_post();
							$thumbnail_class = has_post_thumbnail() ? '' : 'no-thumbnail';
							?>
							<div class="blog-post-container list-layout">
								<div class="blog-post-inner <?php echo esc_attr( $thumbnail_class ); ?>">
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1|Max:6):', 'infinite-news' ); ?></label>
